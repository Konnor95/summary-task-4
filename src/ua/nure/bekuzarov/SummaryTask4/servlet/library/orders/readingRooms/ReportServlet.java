...
/**
 * Responsible for generating report of orders made in reading room(s).
 */
@WebServlet(Actions.Library.ReadingRooms.REPORT)
public class ReportServlet extends LibraryReportServlet {

    @Override
    protected ReportUtil getUtil() {
        return new ReadingRoomsReportUtil(getFileService());
    }

    private final class ReadingRoomsReportUtil extends ReportUtil {

        private ReadingRoomsReportUtil(FileService fileService) {
            super(fileService);
        }
        ...
        protected String getReportTitle(String locale) {
            return getTranslator().translate("text.orders.readingRooms", locale);
        }
